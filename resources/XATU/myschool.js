...
function mergeContiguousSections(courses) {
    ...
    async function runImportFlow() {
        ...
        if (confirm(`✅ 课程导入成功！\n共 ${courses.length} 门课程\n\n是否同时导出纯净JSON文件？\n（需手动设定开学日期，且连续四小节的课默认为两小节，需要手动调整）`)) {
    ...
    (async function bootstrap() {
        try {
            ...
            console.error("[西安工业大学教务适配] 导入失败详情:\n" + details);
    ...
    window.exportXatuCourses = function() {
        try {
            ...
            const timeSlots = JSON.parse(localStorage.getItem('xatu_timeSlots') || '[]');
            
            if (courses.length === 0) {
                alert('❌ 未找到课程数据，请先运行课表导入流程');
                return;
            }
            
            exportCleanJson(courses, timeSlots);
        } catch (e) {
            console.error('手动导出失败:', e);
